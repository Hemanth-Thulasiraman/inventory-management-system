...
import { fetchInventoryById, updateInventory, deleteInventory } from '../store/inventorySlice';
...
export default function InventoryDetail() {
  ...
  useEffect(() => { if (inventory) setStockValue(inventory.currentStock); }, [inventory]);
  ...
  const handleUpdateStock = async () => {
    await dispatch(updateInventory({ id: parseInt(id), currentStock: parseInt(stockValue) }));
    dispatch(fetchInventoryById(id));
  };

  if (!inventory) return <div className="text-center py-8 text-gray-500">Loading...</div>;
    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex items-center space-x-2">
          <button onClick={() => navigate(`/inventory/${id}/edit`)} className="p-2 hover:bg-gray-100 rounded-lg">
      ...
      <div className="bg-white rounded-lg border border-gray-200 p-6 mb-6">
        <div className="flex items-center space-x-4">
          <div className="w-12 h-12 bg-blue-100 rounded-full flex items-center justify-center">
            <span className="text-blue-600 font-semibold text-lg">{product?.name?.charAt(0)}</span>
          </div>
          <div>
            <div className="flex items-center space-x-3">
              <Link to={`/products/${product?.id}`} className="text-xl font-semibold text-blue-600 hover:underline">{product?.name}</Link>
      ...
      <div className="grid grid-cols-4 gap-4 mb-6">
        ...
        <div className="bg-white rounded-lg border border-gray-200 p-4">
          ...
          <p className="text-lg font-semibold text-blue-600">{inventory.currentStock}</p>
        </div>
        <div className="bg-white rounded-lg border border-gray-200 p-4">
